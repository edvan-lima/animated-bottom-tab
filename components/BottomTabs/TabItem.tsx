...
import { getPathXCenterByIndex } from "@/lib/utils"
...
const TabItem: FC<TabProps> = ({
  ...
  const labelContainerStyle = useAnimatedStyle(() => {
    const translateY = animatedActiveIndex.value - 1 === index ? 38 : 100
  ...
  const iconColor = useSharedValue(
    activeIndex === index + 1 ? "#f97316" : "#e5e5e5"
  )
  ...
  useEffect(() => {
    ...
    if (activeIndex === index + 1) {
      iconColor.value = withTiming("#f97316")
    } else {
      iconColor.value = withTiming("#e5e5e5")
    }
  }, [activeIndex])
